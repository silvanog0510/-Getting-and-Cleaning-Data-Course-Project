...
#Remove columns that have variables other than means and standard deviations:
        df_subandact <- as.data.frame(df_merge[,1:2])
        df_merge <- df_merge[,(grep("mean|std",names(df_merge))),drop=FALSE]
        df_merge <- df_merge[,-(grep("meanFreq",names(df_merge))),drop=FALSE]
        df_merge <- cbind(df_subandact$Activity_ID,df_merge)
        df_merge <- cbind(df_subandact$Subject_ID,df_merge)
        colnames(df_merge)[1] <- "Subject_ID"
        colnames(df_merge)[2] <- "Activity_ID"
